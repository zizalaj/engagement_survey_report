...
source(
  here::here("R", "print_report_editable.R"),
  local = TRUE
)
...
copy_path_into_dir <- function(path, destination_dir) {
  if (!file.exists(path) && !dir.exists(path)) {
    stop("Required render asset not found: ", path)
  }
  ...
  if (!all(copied)) {
    stop("Failed to copy required render asset: ", path)
  }
}
...
create_render_workspace <- function(report_path,
  ...
  if (file.exists(token_path)) {
    ...
    token_copied <- file.copy(token_path, token_target, overwrite = TRUE)
    if (!isTRUE(token_copied)) {
      stop("Failed to copy local token.txt into render workspace.")
    }
  }
  ...
  shiny::observeEvent(input$render_report, {
    ...
    render_one_report <- function(output_name, execute_params) {
      ...
      quarto::quarto_render(
        ...
        execute_dir = render_workspace,
        execute_params = execute_params,
        quiet = FALSE
      )

      file.path(render_workspace, output_name)
    }

    render_one_pdf <- function(html_output_name, pdf_output_path, execute_params) {
    ...
    tryCatch({
      ...
      if (needs_department_reports) {
        for (department_name in selected_departments) {
          safe_name <- safe_department_filename(department_name)

          rendered_files <- c(
            rendered_files,
            render_one_pdf(
              html_output_name = paste0("report_oddeleni_", safe_name, ".html"),
              pdf_output_path = file.path(department_output_dir, paste0("report_oddeleni_", safe_name, ".pdf")),
              execute_params = c(
                common_params,
                list(
