...
source(
  here("codex", "navratnost-editor", "R", "typeform_helpers.R"),
  local = knitr::knit_global()
)

token_path <- params$token_path %||% "token.txt"
...
return_rate_inputs <- extract_return_rate_inputs(
  final_long_df,
  celkem_zamestnancu = params$celkem_zamestnancu,
  oddeleni_manual = params$oddeleni_manual
)

zakl_info <- return_rate_inputs$zakl_info
zakl_info_otazky <- return_rate_inputs$zakl_info_otazky
oddeleni_ot <- return_rate_inputs$oddeleni_ot
oddeleni <- return_rate_inputs$oddeleni
odd_souhrn_long <- return_rate_inputs$odd_souhrn_long
avg_doba_vyplneni <- return_rate_inputs$avg_doba_vyplneni
pocet_resp <- return_rate_inputs$pocet_resp
navratnost <- return_rate_inputs$navratnost
celkem_zamestnancu <- params$celkem_zamestnancu %||% NA_real_
...
vek_souhrn <- final_long_df %>%
  ...
  distinct(respondent_id,odpoved_hodnota)%>%
  group_by(odpoved_hodnota)%>%
  summarise(pocet=n())%>%
...
engagement_index_celkem <- final_long_df %>%
  ...
  mutate(odpoved_hodnota = as.numeric(odpoved_hodnota),
         uroven_engagementu = as.factor(case_when(
           ...
           odpoved_hodnota == 3 ~ "Částěčný engagement",
...
engegement_index_otazky <- final_long_df %>%
  filter(group_title == "Engagement") %>%
  mutate(odpoved_hodnota = as.numeric(odpoved_hodnota),
         uroven_engagementu = as.factor(case_when(
           odpoved_hodnota <= 2 ~ "Úplný engagement",
...
<div class="section-title">Drivery engagementu</div>
<div class="section-description">
<p>
Drivery engagementu popisuji, ktere oblasti maji nejsilnejsi vztah k celkove zkusenosti zamestnancu. Pomahaji urcit, na co se zamerit nejdrive.
</p>
<p>
Nasledujici slidy ukazuji, jak drivery cist a jak vypadaji hlavni vysledky na urovni firmy.
</p>
</div>
<div class="scv-progress">
```{r}
...
<!-- Sledovane drivery -->
<div class="slide sledovane-drivery">
<div class="sd-header">
<div class="section-title">Sledovane drivery</div>
<div class="section-description">
V modelu sledujeme oblasti, ktere typicky nejvice souviseji s engagementem. Jejich kombinace vytvari dobry zaklad pro prioritizaci dalsich kroku.
</div>
</div>
<div class="sd-grid">
<div class="sd-card"><span>01</span><strong>Smysl a vyznam prace</strong></div>
<div class="sd-card"><span>02</span><strong>Spoluprace a vztahy</strong></div>
<div class="sd-card"><span>03</span><strong>Vedeni a komunikace</strong></div>
<div class="sd-card"><span>04</span><strong>Rozvoj a rust</strong></div>
<div class="sd-card"><span>05</span><strong>Nastroje a podminky</strong></div>
<div class="sd-card"><span>06</span><strong>Oceneni a uznani</strong></div>
</div>
</div>
